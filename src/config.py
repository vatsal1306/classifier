...
# --- DATASET & DATALOADER ---
DATA_DIR = "dataset/only_statues/training_data"  # Directory where the processed shards are stored
TRAIN_BATCH_SIZE = 64  # Batch size for training (32 human, 32 non-human)
TEST_BATCH_SIZE = 64  # Batch size for validation/testing

# --- MODEL ---
MODEL_NAME = "efficientnet_v2_s"  # Model architecture to use (e.g., "resnet18", "resnet34")
...
SCHEDULER = "CosineAnnealingLR"  # CosineAnnealingLR, CosineAnnealingWarmRestarts
...
DESCRIPTION = "EfficientNet V2 Small SGD on prod data for only statue and painting. Excluding wax statue and occlusion. Experiment with CosineAnnealingLR"  # Description for the current run
RUN_NAME = "effnet_v2_s_prod_statue_coslr"  # Name for the current run (used in the run directory)
